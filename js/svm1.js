    ...
    */
    ...
    SVM.prototype = {
      ...
      train: function(data, labels, options) {
        ...
        var tol = options.tol || 1e-7; // numerical tolerance. Don't touch unless you're pro
        ...
        var maxiter = options.maxiter || 100; // max number of iterations
          ...
          if(alphaChanged == 0) passes++;
            ...
            for(var i=0;i<this.N;i++) {
              if(this.alpha[i] > alphatol) {
                testalphas.push(this.alpha[i]);
              }
          }
          allalpha.push(testalphas);
      ...
      getallAlpha: function(){
        return this.allalpha;
      },
      // inst is an array of length D. Returns margin of given example
      // this is the core prediction function. All others are for convenience mostly
      // and end up calling this one somehow.
      fakemarginOne: function(weight, alphas, inst){
        var f = weight.b;
        for(var i=0;i<this.N;i++) {
          f += alphas[i] * this.labels[i] * this.kernel(inst, this.data[i]);
        }
        return f;
      },
